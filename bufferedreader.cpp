...
read_t buf_t::ensure(unsigned const n)
{
	...
	if (bytes == 0)			return read_t::ERR; // EOF
	else if (bytes < 0)		return read_t::ERR; // read error
	else					return read_t::OK;
}
...
ssize_t buf_t::read(unsigned const n)
{
	...
	while (this->available() < n)
	{
		...
		if (0 == bytes)
			return 0; // EOF and this must be handled.
		bytes_read += bytes;
	}
	return bytes_read;
}
...
/* behavior is almost identical to nb_read_into_buf but it returns -1 on any error
 * and only returns zero in the same circumstances that recv or read would (i.e. EOF)*/
